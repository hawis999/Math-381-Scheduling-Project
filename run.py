"""Test harness: runs solve() across one or all scenarios and reports results."""
...
from solution import Solution, hidden_solution_objective, validate
...
def run_scenario(scenario_dir: Path) -> bool:
    ...
    print(f"  selected:  {len(sol.selected_pairs)} (TA, course) pairs")
    print(f"  wall time: {elapsed:.2f}s")

    ok = True
    vr = validate(sol, data)
    for e in vr.errors:
        print(f"  FAIL: {e}")
    ...
    if sol.objective is not None:
        hidden = hidden_solution_objective(data, scenario_dir)
        if hidden is not None:
            tag = "PASS" if sol.objective >= hidden - 1e-6 else "BELOW"
            print(f"  hidden feasible objective: {hidden:.4f}   "
                  f"(yours: {sol.objective:.4f}  [{tag}])")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run step-1 TA-assignment solve() across scenarios.")
    parser.add_argument("--data-dir", default=str(DEFAULT_DATA_DIR),
                        help=f"directory containing scenario subdirs (default: {DEFAULT_DATA_DIR})")
